...
- **Behavior**: Double back to exit with a \"Press back again to exit\" toast (2s timeout). Sheets (explanation, share, sign-in, journal editor) dismiss first — they push `oneKuralSheet` history entries and bubble-phase listeners dismiss on popstate. On root paths, first back press shows the toast and re-pushes a sentinel (`history.pushState`) to keep the app open. Second back press within 2s drains the sentinel stack with `history.go(-history.length)` and closes the app.
